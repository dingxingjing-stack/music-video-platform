...
class DemucsService:
    ...
    def separate(
        ...
        cmd = [
            ...
            str(input_path),
            "--int24",  # 高质量 24-bit PCM 输出（与 --float32 互斥，只保留一个）
        ]
        ...
        try:
            ...
            # 检查输出文件（demucs 输出目录结构随版本变化，递归查找最稳妥）
            stems = []
            for stem_name in self.STEM_NAMES:
                found = list(temp_output.rglob(f"{stem_name}.wav"))
                if not found:
                    found = list(self.output_dir.rglob(f"{stem_name}.wav"))
                if found:
                    stems.append(str(found[0]))
