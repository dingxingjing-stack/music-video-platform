...
def isolated_db(tmp_path, monkeypatch):
    ...
    # Credits v1：POST /generate 真实扣 30，本组用例只验 ai_limits/流程，注资后不干扰
    install_credits_env(monkeypatch, tmp_path, ("uA", "uB"))
    return db_path
...
def test_retry_stems_limit(isolated_db, fake_modal, disable_bg, monkeypatch):
    ...
    assert c.get(f"/api/v1/ai/task/{tid}/download?file=vocals", headers={"Authorization": "Bearer uA"}).status_code == 409
    ...
    r2 = c.post(f"/api/v1/ai/task/{tid}/retry-stems", headers={"Authorization": "Bearer uA"})
