...
export default function MyWorks() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <p className="text-sm text-muted mb-6">统一管理你创作的所有音频与 MIDI 作品</p>

      <div className="flex gap-2 mb-6">
        {(['all', 'audio', 'midi'] as const).map(t => (
          <button key={t} onClick={() => setFilter(t)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
              filter === t ? 'bg-gradient-to-r from-orange-500 to-pink-500 text-white' : 'bg-elevated text-secondary hover:text-white'
            }`}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-20 card-solid">
          ...
          <p className="text-sm text-muted mb-6">去创作页面生成你的第一个作品吧</p>
          ...
          {filtered.map(work => (
            <div key={work.id} className="card-solid p-4 flex items-center gap-4 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted">{work.created_at} · {work.type === 'audio' ? '音频' : 'MIDI'} · {Math.floor(work.duration / 60)}:{(work.duration % 60).toString().padStart(2, '0')}</p>
              </div>
              <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition">
                {work.audio_url && <button className="btn-base px-3 py-1.5 bg-elevated text-secondary hover:text-white rounded-lg text-xs">▶️ 播放</button>}
                <button className="btn-base px-3 py-1.5 bg-elevated text-secondary hover:text-white rounded-lg text-xs">⬇️ 下载</button>
