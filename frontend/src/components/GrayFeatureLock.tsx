    ...
    <>
      ...
      <div className="relative rounded-xl border border-[#2a2a2a] bg-[#1a1a1a]/80 backdrop-blur-sm p-6 sm:p-8 flex flex-col items-center justify-center text-center min-h-[200px]">
        <div className="absolute top-3 right-3">
          <span className="px-2 py-1 rounded-md bg-[#ff6a10]/10 text-[#ff6a10] text-[10px] font-medium">{t('gray.badge')}</span>
        </div>
        <span className="text-4xl mb-3 opacity-60">{feature.icon}</span>
        <h3 className="text-base font-medium text-white mb-1">{t(feature.name)}</h3>
        <p className="text-xs text-[#888888] mb-3 max-w-sm">{t(feature.description)}</p>
            ...
            >
              {submitted ? (
                <div className="p-10 text-center">
                  ...
                  <p className="mt-3 text-white font-medium">{t('gray.submitted')}</p>
                  <p className="mt-1 text-xs text-[#888888]">{t('gray.review')}</p>
                </div>
              ) : (
                <>
                  <div className="px-5 pt-5 pb-3 border-b border-[#2a2a2a]">
                    <h3 className="font-bold gradient-text text-lg">{t('gray.applyTitle')}</h3>
                    <p className="text-xs text-[#888888] mt-1">{feature.icon} {t(feature.name)} · {t(feature.description)}</p>
                  </div>
                  <div className="p-5 space-y-3">
                    <div>
                      <label className="text-xs text-[#888888] mb-1 block">{t('gray.reason')}</label>
                      <textarea
                        value={reason}
                        onChange={(e) => setReason(e.target.value)}
                        placeholder={t('gray.reasonPlaceholder')}
                        rows={3}
                    ...
                    <div>
                      <label className="text-xs text-[#888888] mb-1 block">{t('gray.contact')}</label>
                      <input
                        value={contact}
                        onChange={(e) => setContact(e.target.value)}
                        placeholder={t('gray.contactPlaceholder')}
                    ...
                    <div className="flex gap-2 pt-2">
                      <button onClick={() => setShowApply(false)} className="flex-1 py-2 rounded-lg text-sm border border-[#2a2a2a] text-[#888888] hover:bg-[#1a1a1a]">{t('gray.cancel')}</button>
                      <button onClick={handleSubmit} disabled={!reason.trim()} className="flex-1 py-2 rounded-lg text-sm font-medium text-white bg-gradient-to-r from-[#ff6a10] to-[#ee0979] disabled:opacity-40">{t('gray.submit')}</button>
                    </div>
                  </div>
                </>
              )}
