...
export function ChordTrackPanel({ onClose }: Props) {
  ...
  useEffect(() => {
    const loadChords = async () => {
      try {
        const response = await fetch(api.url(`/api/v1/chords/library?quality=${selectedQuality === 'all' ? '' : selectedQuality}`));
  ...
  const handleGenerateProgression = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(api.url('/api/v1/chords/generate'), {
        method: 'POST',
        body: formData,
      });
      const data = await response.json();
